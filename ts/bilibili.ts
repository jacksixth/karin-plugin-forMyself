...
// 模拟 Cookie 这里实际使用时请替换为自己的 Cookie 当然这个cookie也能用
const FAKE_COOKIE =
 ...
 * @param text b站链接
 */
const purgeLinkInText = (text: string) =>
  text.replace(/https?:\/\/[-\w~!@#$%&*()+=;':,.?/]+/g, (url) => purgeLink(url))
/**
 * 数字取万
 * @param num 数字
 */
const humanNum = (num: number) =>
...
//直播信息
const getLiveRoomInfo = async (id: string, logger: Logger) => {
  try {
    const response = await axios.get(
    ...
    return [
      ...
      segment.text(
        [
          ...
          `分区: ${parent_area_name}${
            parent_area_name === area_name ? "" : `-${area_name}`
          }`,
...
//专栏信息
const getArticleInfo = async (id: string, logger: Logger) => {
  try {
    const response = await axios.get(
...
//动态信息
const getDynamicInfo = async (id: string, logger: Logger) => {
  try {
    const response = await axios.get(
    ...
    if (code !== 0) return [segment.text(`Error: (${code})${message}`)]
    if (!data?.item) return [segment.text("Error: 无内容")]
...
const formatDynamic = async (item: any) => {
  ...
  const lines = [
    segment.text(
      [`https://t.bilibili.com/${item.id_str}`, `UP: ${author.name}`].join("\n")
    ),
  ]

  const desc = dynamic?.desc?.text?.trim()
  if (desc) lines.push(segment.text(purgeLinkInText(desc)))
