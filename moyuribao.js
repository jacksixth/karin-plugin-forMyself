...
//主动获取摸鱼日报、每日资讯
export const ribao = karin.command(/^#?日报$/, async (e) => {
    if (e.isGroup) {
...
//获取图片地址并发送
const sendImg = async (sendNoList, type) => {
    ...
    sendNoList.forEach(async (sendNo) => {
        let contact;
        if (type === "group")
            contact = karin.contact("group", sendNo);
        else
            contact = karin.contact("friend", sendNo);
        if (myrb && mrzx)
            ...
            karin.sendMsg(karin.getBotAll()[1].account.selfId, contact, segment.text("今天没有摸鱼日报和每日资讯哦"));
        }
    });
};
//获取摸鱼日报、每日资讯图片地址
const getImgAddress = async () => {
    let myrbImg = undefined;
    let mrzxImg = undefined;
    try {
        //摸鱼日报
        ...
        if (myrb.data?.code == 200)
            myrbImg = await downloadImg(myrb.data?.data);
    }
    catch (error) {
        logger.error(error);
    }
    try {
        ...
        if (mrzx.data?.code == 200)
            mrzxImg = await downloadImg(mrzx.data?.data);
    }
    catch (error) {
        logger.error(error);
    }
