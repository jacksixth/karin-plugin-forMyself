...
const data = {};
// 开始游戏 [弹巢]? [模式 1|2]?
export const startgame = karin.command(/^#?(?:开始游戏)(?:\s+(\S+)(?:\s+(\S+))?)?$/, async (e) => {
    if (data[e.groupId]) {
    ...
    data[e.groupId] = save;
    e.reply(`游戏开始，当前为模式为${gameMode == "oneGun" ? "单枪模式" : "多枪模式"}，左轮弹巢可以装${bulletNum}发子弹，现在装了一发，现在弹巢转动起来咯。`);
}, {
    event: "message.group",
    perm: "group.admin",
});
export const turntable = karin.command(/^#?开枪$/, async (e) => {
    ...
    if (save.gameMode == "oneGun") {
        ...
        if (match === save.realBullets) {
            ...
            save.bullets -= 1;
            await e.reply(`砰！枪响了,这一发没装子弹,剩${save.bullets}枪,游戏继续`, {
                reply: true,
            });
            data[e.groupId] = save;
            return true;
        ...
        if (!player) {
            ...
            await e.reply(`加入游戏，你当前可开枪数：${save.players[e.sender.userId].bullets}`, {
                reply: true,
            });
        }
...
export const resetgame = karin.command(/^#?结束游戏$/, async (e) => {
    if (!data[e.groupId]) {
